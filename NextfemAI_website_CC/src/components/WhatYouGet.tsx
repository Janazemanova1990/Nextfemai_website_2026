const cards = [
  {
    ...
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#0e0d12" strokeWidth="2.5">
    ...
    <section className="px-8 py-12 border-b-2 border-nearblack">
      ...
      <div className="grid grid-cols-2 border-2 border-nearblack mb-8">
        {cards.map((card, i) => (
          <div
            key={card.title}
            className={`p-7 bg-white ${i % 2 === 0 ? 'border-r-2 border-nearblack' : ''} ${i < 2 ? 'border-b-2 border-nearblack' : ''}`}
          >
            <div className="mb-4">{card.icon}</div>
            <h3 className="text-[18px] font-bold text-nearblack mb-2">{card.title}</h3>
            <p className="text-[13px] text-muted-body leading-[1.6]">{card.body}</p>
          </div>
        ))}
      </div>
